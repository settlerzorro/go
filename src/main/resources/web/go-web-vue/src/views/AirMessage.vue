  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-cascades"></i> 航班信息
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
...
import { getAirList, commentAir, likeAir, dislikeAir } from "../api/index";
...
export default {
  ...
  setup() {
    ...
    // 获取表格数据
    const getData = () => {
      if (query.aapName && query.fromTime) {
        loading.value = true;
    ...
    const handleGetLike = (index, row) => {
      if (row.likeAirs && row.likeAirs.length > 0) {
        return true;
      }
      return false;
    };
